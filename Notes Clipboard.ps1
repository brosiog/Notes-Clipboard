################### Welcome to the Notes Clipboard Script! This was written by Ambrosio Gomez IV, 2025 #####################
...
# Parameters: $notes is the array that will be sorted
...
            "Alpha" { $newNotes = $notes | Sort-Object }
            "Most Used" { $newNotes = $notes }
      }
...
      $notesModPath = "$PSScriptRoot\notesModified.json"
      $notesDefaultPath = "$PSScriptRoot\notesDefault.json"

      $tabData = [ordered]@{}

      if (Test-Path $notesModPath) {

            $json = Get-Content $notesModPath -Raw | ConvertFrom-Json

      } elseif (Test-Path $notesDefaultPath) {

            $json = Get-Content $notesDefaultPath -Raw | ConvertFrom-Json

      } else {
            Write-Error "Configuration file '$notesPath' not found. Exiting script."
            exit 1
      }
...
            #sends the Notes to be sorted
            $approvedNotes = Get-SortedNotes -notes $tabDict[$tab].approvedNotes
            $denyNotes = Get-SortedNotes -notes $tabDict[$tab].denyNotes
...
      switch ($State) {
            "All 48 States, DC, and Territories" { $correctApprovalList = $approvedHHAmounts["48States"] }
...
                  # adds $7,000 for every added household member over 8
                  $adjustment = ($Household - 8) * 7000
...
                  $status['Message'] ="ELIG - HOUSEHOLD INCOME APPROVED (ANNUAL INCOME IS AT OR UNDER: {0:C}) " -f $threshold
                  $status['BackColor'] = 'YellowGreen'

            } else {

                  $status['Message'] = "ELIG - INCOME TOO HIGH (ANNUAL INCOME IS ABOVE: {0:C}) " -f $threshold
...
      $hhStateLB = New-ListBox -ItemsArray "All 48 States, DC, and Territories", "Alaska", "Hawaii" -X 20 -Y $yPos
...
# Output: User edits are saved to the notesModified.json file
function Save-Usernotes {
...
      #writes the file
      $orderedTabDict | ConvertTo-Json -Depth 10 | Set-Content -Path $notesModPath -Encoding UTF8
}

# Function: Provides a GUI that prompts the user to decide whether they will reset their edits
# Parameters: None
# Output: If user chooses to reset, overwrites with notesModified.json with notesDefault.json
...
            # Define the path for the default notes
            $notesDefaultPath = "$PSScriptRoot\notesDefault.json"

            # Get the contents of the default notes file
            $json = Get-Content $notesDefaultPath -Raw | ConvertFrom-Json
...
            # Define the path for the modified Notes
            $notesModPath = "$PSScriptRoot\notesModified.json"

            # Overwrite the modified Notes with the overwrite dictionary, wich now has the default configuration
            $overwriteDict | ConvertTo-Json -Depth 10 | Set-Content -Path $notesModPath -Encoding UTF8
...
            Save-Usernotes -NewTabDict $tabsDictionary
...
            # saves edits
            Save-Usernotes -NewTabDict $tabDict -Order $newOrder
